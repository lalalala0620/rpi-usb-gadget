...
elif [[ $(ls /etc/netplan/ | wc -l) -gt 0 ]] ; then
sudo sed -i -e '/ethernets/a\        usb0:\n            dhcp4: false\n            addresses: [192.168.0.1/29]\n' /etc/netplan/$(ls /etc/netplan/ -1)
sudo netplan apply
...
You can reach the device on $DEVICE_IP when connected by USB
